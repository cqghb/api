 ...
 * @projectName api
 * @package com.test.api.api.action
 * @className SkuPropertyOptionController
 * @description 产品属性选项关联控制器
 * @auther wangsheng
 * @creatTime 2021/12/19 下午6:15
 * @company 四川省万源市一生活智能科技有限公司
 * @department 小程序-微信小程序
 */
@RestController
@RequestMapping("/skuPropertyOption")
@Api(value = "产品属性选项关联控制器")
public class SkuPropertyOptionController {

    protected static final Logger logger = LoggerFactory.getLogger(SkuPropertyOptionController.class);
     ...
     * @return
     */
    @ApiOperation(value = "产品属性选项关联列表查询", notes = "产品属性选项关联列表查询")
    @PostMapping(value = "/findPage")
    public Result findPage(@RequestBody PageRequest pageQuery) {
        PageResult pageResult = skuPropertyOptionService.findPage(pageQuery);
    ...
    public Result updateDelTag(@RequestBody @Validated CommonIdDto ddDto) {
        TblSkuPropertyOption skuPropertyOption = new TblSkuPropertyOption();
        skuPropertyOption.setId(ddDto.getId());
        int num = skuPropertyOptionService.updateDelTag(skuPropertyOption);
